...
from system_interfaces.msg import Heartbeat, IsImageDegraded, SetBlackboardGroup, Flightphase
...
class BlackboardSetter(ENGELBaseClass):
    ...
    def publish_frequencies(self, topic_name: str, value: List, buffer_length: int = 8):
        ...
        msg = self.create_parameter(
            f"{topic_name[1:].replace('/', '_')}_freq", freq
        )
    ...
    def publish_heartbeat_value(self, topic_name: str, value: dict):
        ...
        msg = self.create_parameter(
                    f"{topic_name[1:].replace('/', '_')}_status", value['current_value']
                )
        ...
        msg = self.create_parameter(
                    f"{topic_name[1:].replace('/', '_').replace('heartbeat', 'lc_state')}", value['lc_state']
                )
        # self.get_logger().info(f"Publish heartbeat status for {topic_name}")
        self.bundle_msg.bb_params.append(msg)
